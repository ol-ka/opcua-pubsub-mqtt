...
static void handleByteString(UA_ByteString* bs) {
    dataReceived = true;
...
perform_publish_subscribe(void *context, void *payloadData, size_t payloadSize, long long int *elapsedTime, bool isEcho){
    ...
    UA_PubSubConnection *connection =
            UA_PubSubConnection_findConnectionbyId(server, pubConnectionIdent);
...
//    }
    ...
    UA_StatusCode ret;
    //ret = UA_NetworkMessage_decodeJson(&networkMessage, encodedBuffer);
    ret = UA_NetworkMessage_decodeBinary(encodedBuffer, &currentPosition, &networkMessage);
...
int main(int argc, char *argv[]) {
    ...
    UA_NetworkAddressUrlDataType networkAddressUrl =
            {UA_STRING_NULL , UA_STRING("opc.mqtt://127.0.0.1:1883/")};
...
//#endif
    ...
    //UA_NetworkAddressUrlDataType pubNetworkAddressUrl = {UA_STRING_NULL , UA_STRING("opc.udp://224.0.0.22:4841/")};
    ...
    UA_PubSubConnection *connection =
            UA_PubSubConnection_findConnectionbyId(server, pubConnectionIdent);
    if(connection != NULL) {
        addSubscriber(server, pubConnectionIdent);
    }
...
/*
    ...
    //UA_NetworkAddressUrlDataType subNetworkAddressUrl =  {UA_STRING_NULL , UA_STRING("opc.udp://224.0.0.22:4840/")};
    //addPubSubConnection(server, &transportProfile, &subNetworkAddressUrl, &subConnectionIdent);
    addPubSubConnection(server, &transportProfile, &networkAddressUrl, &subConnectionIdent);
